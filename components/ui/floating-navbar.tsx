      ...
      >
        {navItems.map((navItem: any, idx: number) => (
          <Link
            key={`link=${idx}`}
            href={navItem.link}
            onClick={() => setActiveTab(navItem?.link)}
            className={cn(
              `relative dark:text-neutral-50 items-center flex space-x-1 text-neutral-600 dark:hover:text-neutral-300 hover:text-neutral-500 ${
                navItem.link === activeTab &&
                "border border-neutral-200 dark:border-white/[0.2] px-4 py-2 rounded-full"
              }`
            )}
          >
            ...
            {navItem.link === activeTab && (
              <span className="absolute inset-x-[50%] -translate-x-[50%] w-1/2 mx-auto -bottom-px bg-gradient-to-r from-transparent via-blue-500 to-transparent  h-px" />
            )}
          </Link>
        ))}
      </motion.div>
